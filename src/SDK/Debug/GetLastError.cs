...
/// <summary>
/// Class used for providing detailed descriptions for an error.
/// </summary>
public class GetLastError
{
    ...
    static readonly string ERROR_CODE_OUTOFRANGE_TXT = "Error code out of range.";
    ...
    const uint PCO_NOERROR = 0;
    // ====================================================================================================== //
    // -- 1. Masks for evaluating error layer, error device and error code: --------------------------------- //
    // ====================================================================================================== //
    const uint PCO_ERROR_CODE_MASK = 0x00000FFF;    // in this bit range the error codes reside
    const uint PCO_ERROR_LAYER_MASK = 0x0000F000;   // in this bit range the layer codes reside
    const uint PCO_ERROR_DEVICE_MASK = 0x00FF0000;  // bit range for error devices / sources 
    const uint PCO_ERROR_RESERVED_MASK = 0x1F000000;// reserved for future use
    const uint PCO_ERROR_IS_COMMON = 0x20000000;    // indicates error message common to all layers
    const uint PCO_ERROR_IS_WARNING = 0x40000000;   // indicates a warning
    const uint PCO_ERROR_IS_ERROR = 0x80000000;     // indicates an error condition

    // ====================================================================================================== //
    // -- 2. Layer definitions: ----------------------------------------------------------------------------- //
    // ====================================================================================================== //
    const uint PCO_ERROR_FIRMWARE = 0x00001000;     // error inside the firmware
    const uint PCO_ERROR_DRIVER = 0x00002000;       // error inside the driver
    const uint PCO_ERROR_SDKDLL = 0x00003000;       // error inside the SDK-dll
    const uint PCO_ERROR_APPLICATION = 0x00004000;  // error inside the application

    // ====================================================================================================== //
    // -- 3.1 FIRMWARE error sources / devices: ------------------------------------------------------------- //
    // ====================================================================================================== //
    const uint SC2_ERROR_POWER_CPLD = 0x00010000;   // error at CPLD in pco.power unit
    const uint SC2_ERROR_HEAD_UP = 0x00020000;      // error at uP of head board in pco.camera
    const uint SC2_ERROR_MAIN_UP = 0x00030000;      // error at uP of main board in pco.camera 
    const uint SC2_ERROR_FWIRE_UP = 0x00040000;     // error at uP of firewire board in pco.camera 
    const uint SC2_ERROR_MAIN_FPGA = 0x00050000;    // error at FPGA of main board in pco.camera 
    const uint SC2_ERROR_HEAD_FPGA = 0x00060000;    // error at FGPA of head board in pco.camera 
    const uint SC2_ERROR_MAIN_BOARD = 0x00070000;   // error at main board in pco.camera
    const uint SC2_ERROR_HEAD_CPLD = 0x00080000;    // error at CPLD of head board in pco.camera
    const uint SC2_ERROR_SENSOR = 0x00090000;       // error at image sensor (CCD or CMOS)
    const uint SC2_ERROR_POWER = 0x000D0000;        // error within power unit
    const uint SC2_ERROR_GIGE = 0x000E0000;         // error at uP of GigE board GigE firmware
    const uint SC2_ERROR_USB = 0x000F0000;          // error at uP of GigE board USB firmware
    const uint SC2_ERROR_BOOT_FPGA = 0x00100000;    // error at Boot FPGA in pco.camera 

    // ====================================================================================================== //
    // -- 3.2 DRIVER devices -------------------------------------------------------------------------------- //
    // ====================================================================================================== //
    const uint PCO_ERROR_DRIVER_FIREWIRE = 0x00300000;// error inside the firewire driver
    const uint PCO_ERROR_DRIVER_USB = 0x00310000;   // error inside the usb driver
    const uint PCO_ERROR_DRIVER_GIGE = 0x00320000;  // error inside the GigE driver
    const uint PCO_ERROR_DRIVER_CAMERALINK = 0x00330000;// error inside the CameraLink driver
                                                        // obsolete (will be removed in a future release):
                                                        // SC2_ERROR_DRIVER renamed to PCO_ERROR_DRIVER_xyz
    const uint SC2_ERROR_DRIVER = 0x000B0000;       // error inside the driver
                                                    // ====================================================================================================== //
                                                    // -- 3.3 SDKDLL devices -------------------------------------------------------------------------------- //
                                                    // ====================================================================================================== //
    const uint PCO_ERROR_PCO_SDKDLL = 0x000A0000;   // error inside the camera sdk dll
    const uint PCO_ERROR_CONVERTDLL = 0x00110000;   // error inside the convert dll
    const uint PCO_ERROR_FILEDLL = 0x00120000;      // error inside the file dll
    const uint PCO_ERROR_JAVANATIVEDLL = 0x00130000;// error inside a java native dll

    // ====================================================================================================== //
    // -- 3.4 Application devices --------------------------------------------------------------------------- //
    // ====================================================================================================== //
    const uint PCO_ERROR_CAMWARE = 0x00100000;      // error in CamWare (also some kind of "device")

    /// <summary>
    /// Converts error code to readable string message.
    /// </summary>
    /// <param name="dwerr">error integer code</param>
    /// <param name="pbuf">error string message</param>
    public static string ToString(int err)
    {
        ...
        switch (layer)   // evaluate layer
        {
            case PCO_ERROR_FIRMWARE:
                {
                    layertxt = "Firmware";
                    devicetxt = device switch
                    {
                        SC2_ERROR_POWER_CPLD => "SC2 Power CPLD",
                        SC2_ERROR_HEAD_UP => "SC2 Head uP",
                        SC2_ERROR_MAIN_UP => "SC2 Main uP",
                        SC2_ERROR_FWIRE_UP => "SC2 Firewire uP",
                        SC2_ERROR_MAIN_FPGA => "SC2 Main FPGA",
                        SC2_ERROR_HEAD_FPGA => "SC2 Head FPGA",
                        SC2_ERROR_MAIN_BOARD => "SC2 Main board",
                        SC2_ERROR_HEAD_CPLD => "SC2 Head CPLD",
                        SC2_ERROR_SENSOR => "SC2 Image sensor",
                        SC2_ERROR_POWER => "SC2 Power Unit",
                        SC2_ERROR_GIGE => "SC2 GigE board",
                        SC2_ERROR_USB => "SC2 GigE/USB board",
                        SC2_ERROR_BOOT_FPGA => "BOOT FPGA",
                        _ => "Unknown device",
                    };
                    break;
                }
            case PCO_ERROR_DRIVER:
                {
                    ...
                    devicetxt = device switch
                    {
                        SC2_ERROR_DRIVER => "pco.camera driver",
                        PCO_ERROR_DRIVER_FIREWIRE => "Firewire driver",
                        PCO_ERROR_DRIVER_USB => "Usb driver",
                        PCO_ERROR_DRIVER_GIGE => "GigE driver",
                        PCO_ERROR_DRIVER_CAMERALINK => "CameraLink driver",
                        _ => "Unknown device",
                    };
                    break;
                }
            case PCO_ERROR_SDKDLL:
                {
                    ...
                    devicetxt = device switch
                    {
                        PCO_ERROR_PCO_SDKDLL => "camera sdk dll",
                        PCO_ERROR_CONVERTDLL => "convert dll",
                        PCO_ERROR_FILEDLL => "file dll",
                        PCO_ERROR_JAVANATIVEDLL => "java native dll",
                        _ => "Unknown device",
                    };
                    break;
                }
